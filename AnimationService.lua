export type AnimationPlayer = Humanoid | AnimationTrack | Animator

local RunService = game:GetService('RunService')

local _LoadedCache : table = {}
local _AnimObjCache : table = {}

local Module : table = {
	LoadedAnimationCache = _LoadedCache,
	AnimObjectCache = _AnimObjCache
}

function Module:AssertAnimationPlayer(AnimationDevice : AnimationPlayer? ) : nil
...
function Module:AssertAnimation(Animation : any?) : Animation?
...
function Module:GetModelAnimator(Model : Instance, bypassGenerate : boolean?) : Animator?
...
function Module:AnimationInput(InputValue : any?, _original : any?) : Animation?
...
function Module:GetPredeterminedAnimations( InputAnimationTable : table ) : table
	local OutputTable : table = { }
	for animName : string, animID : string in pairs( InputAnimationTable ) do
		local inputType : any? = Module:AnimationInput( animID )
...
function Module:GetAnimationTrack(AnimationPlayer : AnimationPlayer?, Animation : Animation?, createAnimator : boolean?, animationProperties : table?) : AnimationTrack?
	...
	local Animator : Animator? = AnimationPlayer:IsA('Animator') and AnimationPlayer or AnimationPlayer:FindFirstChildOfClass("Animator")
	...
	if typeof(animationProperties) == 'table' then
		for k : string, v : any in pairs(animationProperties) do
			LoadedAnimation[k] = v
		end
	end
...
function Module:RunAnimation(AnimationPlayer : AnimationPlayer?, Animation : any, Properties : table?) : AnimationTrack?
	Properties = Properties or {}
	local LoadedAnimation : AnimationTrack? = Module:GetAnimationTrack(AnimationPlayer, Animation, RunService:IsServer(), Properties)
...
function Module:StopAnimation(AnimationObject : Animation?, SpecificAnimationPlayer : AnimationPlayer?) : nil
	...
	for animPlayerObj : AnimationPlayer, animTable : table in pairs(Module.LoadedAnimationCache) do
		if (not SpecificAnimationPlayer) or animPlayerObj == SpecificAnimationPlayer then
			for animObj : Animation, loaded : AnimationTrack in pairs(animTable) do
...
function Module:HasAnimationRunning(AnimationPlayer : AnimationPlayer?, AnimationObject : (Animation | any)?) : boolean
	Module:AssertAnimationPlayer(AnimationPlayer)
	AnimationObject = Module:AnimationInput(AnimationObject)
	assert(typeof(AnimationObject) == "Instance" and AnimationObject:IsA("Animation"), "Invalid Animation Input.")
	local Animator : Animator? = AnimationPlayer:IsA("Animator") and AnimationPlayer or Module:GetModelAnimator(AnimationPlayer, false)
	if Animator then
		for index : number, loadedAnimation : AnimationTrack in pairs(Animator:GetPlayingAnimationTracks()) do
